...
module.exports = {
    name: 'mutelog',
    description: 'Mute log kanalını ayarlar',
    usage: '!mutelog <#kanal>',
    async execute(message, args) {
        ...
        if (args.length < 1) {
            const errorEmbed = new EmbedBuilder()
                ...
                .setDescription('Doğru kullanım: `!mutelog <#kanal>`')
                .addFields(
                    { name: 'Örnek', value: '`!mutelog #mute-log`', inline: true }
                )
                .setTimestamp();
            return message.reply({ embeds: [errorEmbed] });
        }
        ...
        try {
            // MongoDB'ye mute log kanalını kaydet
            const success = await setLogChannel(message.guild.id, 'mute', channel.id);
            ...
            const successEmbed = new EmbedBuilder()
                .setColor('#00ff00')
                .setTitle('✅ Mute Log Kanalı Ayarlandı')
                ...
                .setFooter({ text: 'Artık mute işlemleri bu kanala loglanacak' })
                .setTimestamp();

            await message.reply({ embeds: [successEmbed] });

        } catch (error) {
            console.error('Mute log ayarlama hatası:', error);
            const errorEmbed = new EmbedBuilder()
                .setColor('#ff0000')
                .setTitle('❌ Ayar Hatası')
                .setDescription('Mute log kanalı ayarlanırken bir hata oluştu!')
                .addFields(
